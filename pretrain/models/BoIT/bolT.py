...
from . import BolTransformerBlock
    ...
    def __init__(self, hyperParams, details):
        ...
        for i, layer in enumerate(range(hyperParams.nOfLayers)):
            ...
            if (hyperParams.focalRule == "expand"):
                receptiveSize = hyperParams.windowSize + \
                    math.ceil(hyperParams.windowSize * 2 * i *
                              hyperParams.fringeCoeff * (1-hyperParams.shiftCoeff))
            elif (hyperParams.focalRule == "fixed"):
                receptiveSize = hyperParams.windowSize + \
                    math.ceil(hyperParams.windowSize * 2 * 1 *
                              hyperParams.fringeCoeff * (1-hyperParams.shiftCoeff))
